...
public class Main {
        ...
        public static void menu(String[][] inventory,int itemCounter) throws Exception {
            ...
            switch (choice) {
                ...
                case 0 -> { return;// return-a ме изкарва от метод меню
                }
            }
        ...
        public static void showList(String[][]inventory,int itemCounter) throws Exception {
            for (int i = 0; i < inventory.length; i++) {
                for (int j = 0; j < inventory[i].length; j++) {
                    ...
                    }else if (!inventory[i][j].equals("n/a")) {//за всяки един елемент, който не н/а
        ...
        public static void addItem(String[][]inventory,int itemCounter) throws Exception {
            if (inventory[itemCounter][0].equals("n/a")) {
                for (int j = 0; j < inventory[itemCounter].length; j++) {
                    ...
                    switch (j) {
                        ...
                        case 0 -> {
                            System.out.println();//защо сваляш ред?
            ...
            System.out.println();
            itemCounter++;// Защо?
            menu(inventory,itemCounter);//започва отначало
        }
            ...
            for (int i = 0; i < inventory.length; i++) {
                for (int j = 0; j < inventory[i].length; j++) {
                    ...
                    if (inventory[0][0].equals("n/a")){
                        ...
                        Date itemDate=new SimpleDateFormat("dd.MM.yyyy").parse(inventory[i][2]);//каствам/дате е клас и затова е с главна буква
                        //checks there is an item in between start and end date
                        if (startD.before(itemDate)&&endD.after(itemDate)||startD.equals(itemDate)||endD.equals(itemDate))//ако старт дейта е преди айтем дейта и енд дейта е след айтем дейта или... изпълнявай....
                            switch (j) {
                                case 0 -> System.out.print(inventory[i][0] + " | ");
                                case 1 -> System.out.print("Expiry date: " + inventory[i][1] + " | ");
                                case 2 -> System.out.print("Entry date: " + inventory[i][2] + " | ");
        ...
        public static String[][] initializeInventory() {
            ...
            String[][] inventory = new String[20][8];//Защо?
        ...
        public static void main(String[] args) throws Exception {
            int itemCounter=0; //counts how many items are added to the inventory
